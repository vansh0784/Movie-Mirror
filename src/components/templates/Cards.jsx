...
const Cards = ({ data,title }) => {
    return (
        <div className='w-full flex flex-wrap justify-center gap-4 bg-[#121212]'>
            {data.map((c, i) => (
                <Link to= {`/${c?.media_type || title}/detail/${c?.id}`} key={i} className='relative w-[250px] bg-zinc-950 p-2 rounded shadow-lg mr-3 mb-3'>
                    <img
                    ...
                    {c?.vote_average && !Number.isNaN(c.vote_average) && c.vote_average > 0 ? (
                        <div className='absolute rounded-full text-xl font-semibold p-1 bg-zinc-300 text-gray-800 w-15 h-15 flex items-center justify-center bottom-[25%] right-[-5%]'>
